...
自动获取伦敦金现、布伦特原油、伦敦银现价格，计算金油比和金银比，通过飞书推送精美卡片日报。

## 功能特性

- 自动获取实时行情数据（TradingEconomics）
- 🥇 **金油比**：5 级水平判定（偏低/适中/偏高/极高/极端）
- 🥈 **金银比**：5 级水平判定（偏低/适中/偏高/极高/极端）
- 📊 **数据对比**：表格形式展示昨日、近 7 天、近 1 月、近 1 季涨跌幅
- 时段分离存储：亚盘收盘和美盘收盘独立记录，互不覆盖
- 通过飞书官方 API 推送 Interactive 卡片消息（JSON 2.0 + 表格组件）
- 卡片推送失败时自动降级为纯文本消息
- AES-256-GCM 加密存储，数据永久保存
- 支持定时执行和手动触发
...
**金银比（Gold-to-Silver Ratio）** = 伦敦金现价格 ÷ 伦敦银现价格

金银比反映黄金与白银的相对估值，1970 年至今长期均值约 65:1：

| 金银比 | 判定 | 含义 |
|:-------|:-----|:-----|
| < 50 | 偏低 | 白银相对强势，经济扩张期或通胀预期强 |
| 50 ~ 80 | 适中 | 历史常态区间，贵金属市场相对均衡 |
| 80 ~ 100 | 极高 | 白银被严重低估，避险情绪极端 |
| > 100 | 极端 | 历史罕见，2020 年疫情曾触及 125 |

## 工作流程

```
┌─────────────┐     ┌──────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│  GitHub     │     │  获取实时    │     │  保存数据    │     │  计算比率   │     │  飞书 API   │
│  Actions    │────▶│  行情数据    │────▶│  (时段分离)  │────▶│  生成日报   │────▶│  推送卡片   │
│  定时触发   │     │  (3种商品)   │     │  (AES加密)   │     │  (含数据对比)│    │  消息通知   │
└─────────────┘     └──────────────┘     └──────────────┘     └─────────────┘     └─────────────┘
```
...
2. 爬取 [TradingEconomics](https://zh.tradingeconomics.com/commodity/gold) 获取伦敦金现、布伦特原油、伦敦银现实时价格
3. 按时段（亚盘收盘/美盘收盘）加密保存数据
4. 计算金油比和金银比，分别进行 5 级水平判定
5. 计算多维度数据对比涨跌幅（同类型时段对比）
6. 通过飞书官方 API 发送 Interactive 卡片消息（含表格组件）
...
> 
> **首次部署必须配置 `DATA_ENCRYPT_KEY` 和 `PUSH_TOKEN`**，否则 Actions 无法持久化数据。`FEISHU_BASE_TOKEN` 为可选项，配置后可同步数据到飞书多维表格。

### 3. 手动触发测试

进入 **Actions** 标签页 → 选择 **金油银哨兵** → 点击 **Run workflow**

### 4. 验证推送

检查飞书是否收到金油银哨兵卡片消息。如果未收到，请参考下方 [常见问题](#常见问题) 排查。
...
├── data_store.py                 # 数据存储模块（AES-256-GCM 加密 + 多维度对比计算）
├── gold_oil_data.enc             # 加密数据文件（自动生成，提交到仓库）
├── gold_oil_data.json            # 明文数据文件（本地开发用，已加入 .gitignore）
├── requirements.txt              # Python 依赖（requests, cryptography）
├── .gitignore                    # Git 忽略规则
└── README.md                     # 项目文档
```
...
| `fetch_silver_price()` | 爬取 TradingEconomics 获取伦敦银现价格和涨跌幅 |
| `calculate_ratio()` | 计算金油比（黄金价格 ÷ 原油价格） |
| `calculate_gs_ratio()` | 计算金银比（黄金价格 ÷ 白银价格） |
| `get_current_session()` | 根据北京时间自动判断当前时段（亚盘收盘/美盘收盘） |
| `generate_report()` | 生成飞书 Interactive 卡片 JSON（含表格组件）和纯文本降级内容 |
| `FeishuPusher` | 飞书消息推送器，支持卡片消息和纯文本消息，含重试机制 |
| `FeishuBaseWriter` | 飞书多维表格写入器，每次运行同步数据到多维表格 |
| `add_record()` | 按日期+时段存储贵金属数据（含金油比和金银比） |
| `get_multi_period_changes()` | 计算多维度数据对比涨跌幅（支持金油比/金银比） |
| `get_data_summary()` | 获取数据汇总信息（总记录数、日期范围等） |
...
TradingEconomics 可能会更新页面结构，导致正则匹配失败。此时需要检查并更新 `fetch_gold_price()`、`fetch_brent_oil_price()` 和 `fetch_silver_price()` 中的匹配规则。
...
| cryptography | AES-256-GCM 数据加密 |
| GitHub Actions | 定时任务调度 |
| 飞书开放平台 API | Interactive 卡片消息推送（JSON 2.0 + 表格组件） |

## 免责声明

本工具仅供研究参考，不构成投资建议。金油比和金银比分析基于历史数据和公开信息，实际市场情况请以专业机构分析为准。
