...
如果只想验证行情数据获取是否正常，可以临时注释掉飞书推送部分，或在未配置凭证的情况下运行，程序会输出获取到的价格和金油比计算结果。
...
| `get_current_session()` | 根据北京时间自动判断当前时段（亚盘/美盘/欧盘） |
...
| `get_multi_period_changes()` | 计算多时段数据对比涨跌幅 |
...
| 对比维度 | 说明 | 数据积累时间 |
|:---------|:-----|:-----------|
| 上一时段 | 与上一个时段对比 | 立即（第二次运行后） |
| 近 7 时段 | 与近 7 个时段平均值对比 | 约 3-4 天 |
| 近半月 | 与近 20 个时段平均值对比 | 约 10-14 天 |
| 近 1 月 | 与近 60 个时段平均值对比 | 约 1 个月 |
